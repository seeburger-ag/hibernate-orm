...
public class HibernateBundleActivator implements BundleActivator {
	...
	public void start(BundleContext context) throws Exception {
		...
		Bundle hibernateCoreBundle = FrameworkUtil.getBundle( Session.class );
		Bundle hibernateEntityManagerBundle = FrameworkUtil.getBundle( HibernatePersistenceProvider.class );
		if (hibernateCoreBundle == null) {
		    // in JBOSGi hibernate is not deployed as bundle, but as JBoss-module
		    osgiClassLoader.addClassLoader(Session.class.getClassLoader());
		}
		else {
		    osgiClassLoader.addBundle( hibernateCoreBundle );
		}
		if (hibernateEntityManagerBundle == null) {
		    // in JBOSGi hibernate is not deployed as bundle, but as JBoss-module
            osgiClassLoader.addClassLoader(HibernatePersistenceProvider.class.getClassLoader());
		}
		else {
		    osgiClassLoader.addBundle( hibernateEntityManagerBundle );
		}
